...
Better UI Imropvement 1.4.17 is a renderer-only plugin for [Codex Script Loader](https://github.com/JHees/codex-script-loader). It brings project-aware sidebar styling, reliable quota display, focused formula and local-image preview support, compacted-context title regeneration, native thread export and permanent deletion, and a dedicated settings page into one installable script.

Version 1.4.17 restores project and conversation colors after Codex's responsive sidebar update, including project titles and icons. Saved colors and **No color** choices are preserved, and reloads clear duplicate quota-alert styles.

> [!IMPORTANT]
> **Codex++ support ended with version `1.2.4`.** That is the final version published in the BigPizzaV3 Codex++ Script Market. Versions after `1.2.4`, including the current `1.4.17`, target Codex Script Loader. No new releases, compatibility fixes, or testing are provided for Codex++.
