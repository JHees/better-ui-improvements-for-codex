...
Better UI Imropvement 1.4.19 is a renderer-only plugin for [Codex Script Loader](https://github.com/JHees/codex-script-loader). It brings project-aware sidebar styling, reliable quota display, focused formula and local-image preview support, compacted-context title regeneration, native thread export and permanent deletion, and a dedicated settings page into one installable script.

Version 1.4.19 adapts to Codex's updated sidebar and split renderer modules. Usage sits compactly in the navigation rail, shows reset time on hover, and leaves the chat list at full height. Native thread actions, project-color menus, and Markdown formula previews are restored. See the [release notes](docs/RELEASE-1.4.19.md).

> [!IMPORTANT]
> **Codex++ support ended with version `1.2.4`.** That is the final version published in the BigPizzaV3 Codex++ Script Market. Versions after `1.2.4`, including the current `1.4.19`, target Codex Script Loader. No new releases, compatibility fixes, or testing are provided for Codex++.
